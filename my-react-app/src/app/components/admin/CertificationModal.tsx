...
export function CertificationModal({ isOpen, onClose, certification }: CertificationModalProps) {
  ...
  // Internal state remains with 'year' for easier UI management
  ...
  useEffect(() => {
    if (certification) {
      setFormData({
        ...
        year: certification.date, // Mapping 'date' from context to 'year' in form
        url: '', // Add this if you add 'url' to your Certification interface later
      });
    } else {
      setFormData({
        title: '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Mapping formData to the Certification interface (year -> date)
    const submissionData = {
      title: formData.title,
      issuer: formData.issuer,
      date: formData.year, // Using 'date' as required by PortfolioContext
    };

    try {
      if (certification) {
        await updateCertification(certification.id, submissionData);
        toast.success('Certification updated successfully');
      } else {
        await addCertification(submissionData);
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              ...
              <form onSubmit={handleSubmit} className="p-8 space-y-6">
                <div>
                  <Label htmlFor="title" className="text-slate-300 mb-2 block">Certification Title *</Label>
                  <Input
                    id="title"
                    required
                ...
                <div>
                  <Label htmlFor="issuer" className="text-slate-300 mb-2 block">Issuing Organization *</Label>
                  <Input
                    id="issuer"
                    required
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  <div>
                    <Label htmlFor="year" className="text-slate-300 mb-2 block">Year *</Label>
                    <Input
                      id="year"
                      required
                      value={formData.year}
                  ...
                  <div>
                    <Label htmlFor="url" className="text-slate-300 mb-2 block">Certificate URL</Label>
                    <Input
                      id="url"
                      type="url"
                      value={formData.url}
